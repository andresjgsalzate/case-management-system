...
interface TodoPriorityEditModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export const TodoPriorityEditModal: React.FC<TodoPriorityEditModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  priority,
  ...
  const [formData, setFormData] = useState<UpdateTodoPriorityRequest>({
    name: "",
    description: "",
  ...
  // Actualizar form data cuando cambie la prioridad
  useEffect(() => {
    if (priority) {
      setFormData({
        name: priority.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await todoPriorityService.updatePriority(priority.id, {
        ...formData,
      ...
      if (response.success) {
        ...
        throw new Error(response.message || "Error al actualizar la prioridad");
      }
    } catch (err) {
      const errorMessage =
  ...
  const handleInputChange = (
    field: keyof UpdateTodoPriorityRequest,
    value: string | number
  ) => {
